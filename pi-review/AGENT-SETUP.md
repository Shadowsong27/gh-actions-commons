...
**2. The default model chain will not work for you.** The `pi-models` default
(`litellm-responses/gpt-5.6-terra-medium,litellm/deepseek-v4-pro`) points at the
maintainer's private LiteLLM gateway. Anywhere else **none of those names resolve**, every
entry in the chain fails, and the reviewer posts `⚠️ REVIEW DID NOT RUN` and turns the
check red on every PR. **You must set `pi-models` to models the target runner's `pi` can actually
resolve.** This is the single most common way this install fails.
...
**If you cannot determine any valid model name, ask the user.** Do not guess, and do not
leave the default in place — see the warning above.
...
These are the inputs worth setting; the workflow declares a few more escape hatches with
sane defaults that you should not need to touch.

| Input | Default | Use when |
|-------|---------|----------|
| `pi-models` | the maintainer's private chain | **Always set this.** |
